...
class Select2 extends Html {
	...
	// API endpoints
	const URL_FAM  = 'autocomplete.php?field=S2-FAM';
	const URL_INDI = 'autocomplete.php?field=S2-INDI';
	const URL_NOTE = 'autocomplete.php?field=S2-NOTE';
	const URL_OBJE = 'autocomplete.php?field=S2-OBJE';
	const URL_REPO = 'autocomplete.php?field=S2-REPO';
	const URL_SOUR = 'autocomplete.php?field=S2-SOUR';
	const URL_SUBM = 'autocomplete.php?field=S2-SUBM';
	...
	public static function familyValue(Family $family) {
		return strip_tags($family->getFullName());
	}
	...
	public static function individualValue(Individual $individual) {
		return strip_tags($individual->getFullName() . ', ' . $individual->getLifeSpan());
	}
		...
		while (is_object($row = $cursor->fetch())) {
			...
			if ($individual !== null && $individual->canShowName()) {
				if ($offset > 0) {
					...
					$results[] = [
						'id'   => $row->xref,
						'text' => str_replace(['@N.N.', '@P.N.'], [I18N::translateContext('Unknown surname', '…'), I18N::translateContext('Unknown given name', '…')], $row->n_full) . ', ' . strip_tags($individual->getLifeSpan()),
					];
				}
			}
		}
	...
	public static function mediaObjectValue(Media $media) {
		return strip_tags($media->getFullName() . ', ' . basename($media->getFilename()));
	}
	...
	public static function noteValue(Note $note) {
		return strip_tags($note->getFullName());
	}
	...
	public static function repositoryValue(Repository $repository) {
		return strip_tags($repository->getFullName());
	}
	...
	public static function sourceValue(Source $source) {
		return strip_tags($source->getFullName());
	}
	...
	public static function submitterValue(GedcomRecord $submitter) {
		return strip_tags($submitter->getFullName());
	}
