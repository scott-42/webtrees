...
abstract class AbstractTheme {
	...
	public function assetUrl() {
		return 'themes/_common/css-1.8.0/';
	}
	...
	protected function formQuickSearch() {
		if ($this->tree) {
			return
				'<form action="search.php" class="wt-header-search" role="search">' .
	...
	protected function formQuickSearchFields() {
		return
			...
			'<input type="search" class="form-control form-control-sm" id="quick-search" name="query" size="15" placeholder="' . I18N::translate('Search') . '">' .
	...
	protected function formatTreeTitle() {
		if ($this->tree) {
			return '<h1 class="wt-site-title">' . $this->tree->getTitleHtml() . '</h1>';
		} else {
			return '';
		}
	}
	...
	protected function logoHeader() {
		return '<div class="wt-site-logo"></div>';
	}
	...
	protected function stylesheets() {
		$stylesheets = [
			...
			WT_SELECT2_CSS_URL,
			'themes/_common/css-1.8.0/style.css',
